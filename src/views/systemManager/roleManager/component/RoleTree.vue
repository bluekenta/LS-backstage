...
const initTreeList = async () => {
  loading.value = true;
  const res = await API.allPermissionList({ roleId });
  loading.value = false;
...
//- 判断是否为半选中状态
const filterHalfChecked = (menusData: RoleAPI.ChildResourceList[]) => {
...
const saveRole = async () => {
  ...
  if (!res.code) closeDialog();
};

//- 获取所有的子节点
const getAllNodeKeys = (node: RoleAPI.ChildResourceList) => {
  let keys = [node.id];
