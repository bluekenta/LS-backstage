<template>
  ...
  >
    <el-form-item prop="other">
      <el-input
        :row="4"
        v-model="newData.orderDetailRemark"
        :placeholder="t('请输入备注')"
        clearable
        class="!w-[400px]"
        maxlength="255"
...
const props = defineProps<{
  data: { orderDetailId: number; orderDetailRemark: string };
}>();
...
const confirmClick = () => {
  API.editMatchDetailRemark({
    orderDetailId: newData.orderDetailId,
    remark: newData.orderDetailRemark
  }).then(res => {
    if (res.code === 0) {
      emits('closeDialog');
    } else {
