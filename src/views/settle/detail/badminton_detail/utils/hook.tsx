...
export function useBilliardsHook() {
  ...
  const onSearch = async () => {
    const res = await API.getBadmintonEvents({
  ...
  const resetBtnClick = async (row: SattleDataAPI.volleyballData) => {
    ElMessageBox.confirm(t('确定要重新结算该事件么？'), {
      ...
      loading.value = true;
      const res = await API.badmintonReSettleMatchEvent({
        matchId,
        id: row.id,
  ...
  const settlementHandicap = async (row: SattleDataAPI.TennisEventsData) => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        const res = await API.addBadmintonballEvent(params);
        loading.value = false;
        message(res.msg, { type: res.code ? 'error' : 'success' });
        if (!res.code) onSearch();
      } catch (error) {
  ...
  const allSettleBtnClick = async () => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        ...
        const res = await API.badmintonManualSettlementFullTime({ matchId });
  ...
  const openSecondSettleClick = async () => {
    const res = await API.badmintonOpen2ndSettlementStatus({ matchId });
    message(res.msg, { type: res.code ? 'error' : 'success' });
    if (!res.code) {
      getMatchDetail();
      saveCanceList();
  ...
  const confirmSecondSettleClick = async () => {
    ElMessageBox.confirm(
      ...
      const res = await API.badmintonConfirm2ndSettlement(confirmParams);
      if (res.code) return message(res.msg, { type: 'error' });
