...
import { columns, MATCH_PERIOD_TYPES } from './tableColumnList';
...
export function useDetailHook() {
  ...
  const onSearch = async () => {
    const res: SattleDataAPI.getHandballEventRes = await API.getIceHockeyEvents(
      {
  ...
  const addEventRow = () => {
    ...
    const params: Partial<SattleDataAPI.volleyballData> = {
      matchId,
      matchPeriodId: dataList.length > 2 ? 40 : dataList.length + 1,
      isEdit: false,
      firstT1: '',
      firstT2: '',
  ...
  const settlementHandicap = async (row: SattleDataAPI.TennisEventsData) => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        const res = await API.addIceHockeyEvent(params);
        loading.value = false;
  ...
  const allSettleBtnClick = async () => {
    ...
    if (dataList.length < 2 || dataList.some(item => !item.id) || s1 === 0) {
      return message(t('请先完局结算'), { type: 'error' });
    }
    ElMessageBox.confirm(
      `
      <div class="text-center">
      ...
      try {
        ...
        const res = await API.allIceHockeySettlement({ matchId });
  ...
  const resetBtnClick = async (row: SattleDataAPI.volleyballData) => {
    ElMessageBox.confirm(t('确定要重新结算该事件么？'), {
      ...
      loading.value = true;
      const res = await API.iceHockeyReSettleMatchEvent({
        matchId,
        id: row.id,
        matchPeriodId: row.matchPeriodId,
        firstT1: row.firstT1,
  ...
  const openSecondSettleClick = async () => {
    const res = await API.iceHockeyOpen2ndSettlementStatus({ matchId });
    message(res.msg, { type: res.code ? 'error' : 'success' });
    if (!res.code) {
      getMatchDetail();
      saveCanceList();
  ...
  const confirmSecondSettleClick = async () => {
    ElMessageBox.confirm(
      ...
      const res = await API.iceHockeyConfirm2ndSettlement(confirmParams);
      if (res.code) return message(res.msg, { type: 'error' });
