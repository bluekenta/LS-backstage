<template>
  <div v-if="!row.id || renderObj.reSettleStatus === 1">
    <el-button
      size="small"
      type="primary"
      v-if="!row.isEdit"
    ...
    <el-button
      ...
      >{{ row.matchPeriodId === 40 ? t('结算加时赛') : t('结算局') }}</el-button
    >
  ...
  >
    <el-button
      v-if="!row.isEdit && hasAuth('RECALCULATIONANDSETTLE')"
      size="small"
      v-auth="['RECALCULATIONANDSETTLE']"
      type="primary"
      @click="resetClick"
      >{{ t('开启重新结算') }}</el-button
    >
    <el-button v-else-if="hasAuth('RECALCULATIONANDSETTLE')" size="small" type="primary" @click="resetBtnClick">{{
      t('重新结算')
    }}</el-button>
  </template>
  <span v-else>--</span>
