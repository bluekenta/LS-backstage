...
export function useVolleballHook() {
  ...
  const resetBtnClick = async (row: SattleDataAPI.volleyballData) => {
    ElMessageBox.confirm(t('确定要重新结算该事件么？'), {
      ...
      loading.value = true;
      const res = await API.volleyballReSettleMatchEvent({
        matchId,
        id: row.id,
  ...
  const allSettleBtnClick = async () => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        ...
        const res = await API.volleyballManualSettlementFullTime({ matchId });
  ...
  const openSecondSettleClick = async () => {
    const res = await API.volleyballOpen2ndSettlementStatus({ matchId });
    message(res.msg, { type: res.code ? 'error' : 'success' });
    if (!res.code) {
      getMatchDetail();
      saveCanceList();
  ...
  const confirmSecondSettleClick = async () => {
    ElMessageBox.confirm(
      ...
      const res = await API.volleyballConfirm2ndSettlement(confirmParams);
      if (res.code) return message(res.msg, { type: 'error' });
