...
export function useBilliardsHook() {
  ...
  const onSearch = async () => {
    const res = await API.getPingpongEvents({
  ...
  const resetBtnClick = async (row: SattleDataAPI.volleyballData) => {
    if (+row.firstT1 === +row.firstT2)
      return message('乒乓球比分无法输入平局，请再次确定结果', {
        type: 'error'
      });
    ElMessageBox.confirm(t('确定要重新结算该事件么？'), {
      ...
      loading.value = true;
      const res = await API.pingPongReSettleMatchEvent({
        matchId,
        id: row.id,
  ...
  const settlementHandicap = async (row: SattleDataAPI.TennisEventsData) => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        const res = await API.addPingPongballEvent(params);
        loading.value = false;
        message(res.msg, { type: res.code ? 'error' : 'success' });
        if (!res.code) onSearch();
      } catch (error) {
  ...
  const allSettleBtnClick = async () => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        ...
        const res = await API.PingPongManualSettlementFullTime({ matchId });
  ...
  const openSecondSettleClick = async () => {
    const res = await API.pingPongOpen2ndSettlementStatus({ matchId });
    message(res.msg, { type: res.code ? 'error' : 'success' });
    if (!res.code) {
      getMatchDetail();
      saveCanceList();
  ...
  const confirmSecondSettleClick = async () => {
    ElMessageBox.confirm(
      ...
      const res = await API.pingPongConfirm2ndSettlement(confirmParams);
      if (res.code) return message(res.msg, { type: 'error' });
