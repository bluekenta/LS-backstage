...
export function useBaseballHook() {
  ...
  const onSearch = async () => {
    const res = await API.getBaseballEvents({ matchId });
    loading.value = false;
  ...
  const addEventRow = () => {
    ...
    if (!noEmpty) return message(t('请先结算之前的局赛事'), { type: 'error' });
  ...
  const resetBtnClick = async (row: SattleDataAPI.volleyballData) => {
    ElMessageBox.confirm(t('确定要重新结算该事件么？'), {
      ...
      loading.value = true;
      const res = await API.baseballReSettleMatchEvent({
        matchId,
        id: row.id,
  ...
  const settlementHandicap = async (row: SattleDataAPI.TennisEventsData) => {
    ...
    ElMessageBox.confirm(
      ...
      try {
        const res = await API.addBaseballEvent(params);
        loading.value = false;
        message(res.msg, { type: res.code ? 'error' : 'success' });
        if (!res.code) onSearch();
      } catch (error) {
  ...
  const showErrorDialog = () => {
    ElMessageBox.confirm(
      <div>
        <div class="font-bold">{t('未满足局数和比分要求')}</div>
        <div>
          {t('最少进行7局后任意一队领先10分，或最少进行5局后任意一队领先15分')}
        </div>
      </div>,
      t('错误提示'),
      {
        ...
        type: 'error',
        customClass: 'confirm_settle_dialog',
  ...
  // - 是否可以全场结算判断
  const hasItPassedTheFullSettlementInspection = () => {
    let s1 = 0;
    let s2 = 0;
    ...
    if (
      (len < 7 && len >= 5 && Math.abs(s1 - s2) < 15) ||
      (len < 9 && len >= 7 && Math.abs(s1 - s2) < 10)
    ) {
      showErrorDialog();
      return false;
    }
  ...
  const allSettleBtnClick = async () => {
    if (!hasItPassedTheFullSettlementInspection()) return;
    ElMessageBox.confirm(
      `
      <div class="text-center">
      ...
      try {
        ...
        const res = await API.baseballManualSettlementFullTime({ matchId });
  ...
  const openSecondSettleClick = async () => {
    const res = await API.baseballOpen2ndSettlementStatus({ matchId });
    message(res.msg, { type: res.code ? 'error' : 'success' });
    if (!res.code) {
      getMatchDetail();
      saveCanceList();
  ...
  const confirmSecondSettleClick = async () => {
    ElMessageBox.confirm(
      ...
      const res = await API.baseballConfirm2ndSettlement(confirmParams);
      if (res.code) return message(res.msg, { type: 'error' });
