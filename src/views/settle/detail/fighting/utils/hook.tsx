...
export function useBilliardsHook() {
  ...
  const onSearch = async () => {
    const res = await API.getBoxingEvents({ matchId });
    loading.value = false;
  ...
  const addEventRow = () => {
    ...
    if (!noEmpty) return message(t('请先结算之前的事件'), { type: 'error' });

    const params: Partial<SattleDataAPI.volleyballData> = {
      matchId,
      matchPeriodId: dataList.length + 1,
      isEdit: false,
      firstT1: '',
      firstT2: '',
      secondNum: null,
  ...
  const resetBtnClick = async (row: SattleDataAPI.volleyballData) => {
    ElMessageBox.confirm(t('确定要重新结算该事件么？'), {
      ...
      loading.value = true;
      const res = await API.boxingReSettleMatchEvent({
        matchId,
        id: row.id,
  ...
  //- 结算回合事件
  const settlementHandicap = async (row: SattleDataAPI.volleyballData) => {
    const { firstT1, firstT2, firstNum } = row;
    if (firstT1 === '' || firstT2 === '')
    ...
    ElMessageBox.confirm(
      ...
      `${t('结算第')}${firstNum}${t('回合')}`,
      {
        ...
        cancelButtonText: t('取消'),
        center: true,
        customClass: 'settle_confirm_dialog'
      }
    ).then(async () => {
      loading.value = true;
      const params = {
      ...
      try {
        const res = await API.addBoxingEvent(params);
        loading.value = false;
        message(res.msg, { type: res.code ? 'error' : 'success' });
        if (!res.code) onSearch();
      } catch (error) {
  ...
  //- 获取获胜方
  const getWinner = (row: SattleDataAPI.volleyballData) => {
    return row.firstT1 > row.firstT2
      ? 'home'
      : row.firstT1 < row.firstT2
      ? 'away'
      : '';
  };
  ...
  const allSettleBtnClick = async () => {
    let s1 = 0;
    let s2 = 0;
    const scoreList = dataList.map(item => [item.firstT1, item.firstT2]);
    ...
    ElMessageBox.confirm(
      ...
      try {
        ...
        const res = await API.boxingManualSettlementFullTime({ matchId });
  ...
  const openSecondSettleClick = async () => {
    const res = await API.boxingOpen2ndSettlementStatus({ matchId });
    message(res.msg, { type: res.code ? 'error' : 'success' });
    if (!res.code) {
      getMatchDetail();
      saveCanceList();
    ...
    ElMessageBox.confirm(
      ...
      confirmParams.newEventList = dataList.map(item => {
        return {
          ...
          homeAway: getWinner(item)
        };
      });
      const res = await API.boxingConfirm2ndSettlement(confirmParams);
      if (res.code) return message(res.msg, { type: 'error' });
